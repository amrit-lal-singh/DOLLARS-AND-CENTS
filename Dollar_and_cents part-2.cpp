...
void check_if_increased(vector<int> v,vector<int> &value_at_index,int index,int dollars,bool &increased,n){ //v is passed as copy, increased is passed by reference
        ...
        //swap all allowed indexes with the given index and see if there is any increase in dollars
            ...
            for(int i=0;i<v.size();i++){
                ...
                swap(v[index],v[i]);
                print_array(v);
                int new_cents,new_dollars,x;
                cin>>x;

                new_dollars = x-n;
                new_cents = n - new_dollars;
        ...
        vector<int> v(n),value_at_index(n,-1); // value_at_index[i]=-1 means unknown value
            ...
            if(new_dollars==dollars-2){
                ...
                if(new_dollars==dollars-1){
                        swap(v[index],v[i]);
                check_if_increased(v,value_at_index,index,dollars,increased,n);
                }
                else {
                    check_if_increased(v,value_at_index,index,new_dollars,increased,n);
                }
